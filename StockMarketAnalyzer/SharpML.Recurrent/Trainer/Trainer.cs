            ...
            foreach (DataSequence seq in sequences)
            {
                ...
                foreach (DataStep step in seq.Steps)
                {
                    ...
                    if (step.TargetOutput != null)
                    {
                        ...
                        if (applyTraining)
                        {
                            lossTraining.Backward(output, step.TargetOutput); //рассчитывает ошибку на выходе у выходных  нейронов, т.е. DW
                        }
                    }
                }
                ...
                if (applyTraining)
                {
                    g.Backward(); //backprop dw values вычисление DW
                    UpdateModelParams(network, learningRate); //update params вычисление новых весов и биаз 
                }
            }
